...
const getPrices = async (amountInFrontend) => {
  ...
  provider.on('block', async (blockNumber) => {
    ...
    const contractToken = new ethers.Contract(ADDRESS_FROM_WETH_POLY_MAIN, erc20ABI, provider);
    ...
    const amountIn = ethers.utils.parseUnits(amountInFrontend, decimals);
      ...
      // Output the amount
      const amount = ethers.utils.formatUnits(amountOutFrontendUNIV3WETH_IN_DAI.toString(), 18);
      //return amount; 

        // Log output
    if (!DAI_found) { DAI_found = true; data += `${Math.floor(parseFloat(amount) * 10000) / 10000},`; }
    console.log('UNI-V3 - POLYGON-MAINNET : WETH_IN_USDC', amount);
    ...
    console.log('SUSHI - POLYGON-MAINNET : WETH_IN_USDC', amountOutFrontendSUSHI_WETH_IN_DAI);
    if (!sushi_found) {
      sushi_found = true;
